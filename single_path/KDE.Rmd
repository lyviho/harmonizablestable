...
a_list <- list("075","150")
n_list <- list("_0","_1","_2","_3")
N_list <- list(c(10,20,50),
               c(50,75,100),
               c(100,200,300),
               c(300,500,1000)) 
...
for(i in 1:4){
  ...
  for(j in 1:1){
    ...
    for(k in 3:4){
      n_str <- n_list[[k]]
      mat <- readMat(paste("matdata/f",i,"a",a_str,n_str,"_zp.mat", sep=""))
      Zhat <- mat$Z.hat
      teval <- mat$t.eval
      feval <- mat$f.eval
      fval <- mat$fval
      ...
      for(N in N_list[[k]]){
        Zhat_restr <- Zhat[1:N]
        Zhat_restr_extend <- c(-Zhat_restr,Zhat_restr)
        kde_restr <- ggplot() + 
        geom_density(mapping=aes(Zhat_restr, color = "KDE"), 
                   kernel="gaussian",
                   bw = "SJ", 
                   # color = rgb(0, 0.4470, 0.7410),
                   lwd = 1)
        kdedata <- ggplot_build(kde_restr)
        x <- kdedata$data[[1]]$x
        x_delta <- x[2]-x[1]
        ...
        L2dist <- rbind(L2dist,
                        tibble(alpha=mat$a,f_i=i,n=mat$n, N=N,L2_dist=dist))
      }
    }
  }
}
L2dist 
round(filter(L2dist,f_i[]==4)$L2_dist*10,digits=2)
```
